...
class TargetEnv3D(base_env.EnvBase):
    NAME = 'TargetEnv3D'
    ...
    def calc_potential(self):
        target_delta, target_height_delta = self.get_target_delta_and_angle()
        self.linear_potential = -target_delta.norm(dim=1).unsqueeze(1)
        self.height_dist = target_height_delta
        #self.angular_potential = target_angle.cos()

    def get_target_delta_and_angle(self):
        
        target_delta = self.target[:,:2] - self.root_xz
        target_z_delta = self.target[:,2:3] - self.root_y
        #print(self.target.shape, self.root_y.shape,target_xy_delta.shape, target_z_delta.shape)
        #target_delta = torch.cat([target_xy_delta, target_z_delta], dim=-1)
        return target_delta, target_z_delta

    def get_observation_components(self):
        target_delta, target_z_delta = self.get_target_delta_and_angle()
        #Should be negative because going from global to local
        mat = self.get_rotation_matrix(-self.root_facing, dim=2)
        #print(mat.shape, target_delta.shape)
        delta = (mat * target_delta.unsqueeze(1)).sum(dim=2)
        delta = torch.cat([delta, target_z_delta],dim=-1)
    ...
    def reset_target(self, indices=None, location=None):
        if location is None:
            ...
            if indices is None:
                ...
                self.target[:, 2].uniform_(*self.arena_height)
            else:
                ...
                new_heights = self.target[indices, 2].uniform_(*self.arena_height)
                self.target[:, 2].index_copy_(dim=0, index=indices, source=new_heights)
    ...
    def calc_env_state(self, next_frame):
        ...
        target_xy_dist = -self.linear_potential
        target_h_dist = self.height_dist
        target_is_close = (target_xy_dist < 0.6) * (target_h_dist < 0.25)
        ...
        self.reward.add_(target_is_close.float() * 30.0)
